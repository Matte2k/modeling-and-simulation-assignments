function [solution,converge,info] = newton(f, xGuess, method ,toll, nmax , printFlag, plotFlag)
%newtonSym
%   Function that compute the zero of given function using the Newton's
%   method. The jacobian can be computed in 3 different ways:

%%% Prelimirary check
if nargin == 5      % default options with 5 input
    printFlag = true;
    plotFlag = false;
end

if nargin == 3      % default options with 3 input
    toll = 1e-12;
    nmax = 1e3;
    printFlag = true;
    plotFlag = false;
