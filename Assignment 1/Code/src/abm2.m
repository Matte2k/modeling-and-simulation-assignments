...
%ABM2 Summary of this function goes here
...
switch abmOptions.method
    ...
    case 'Custom'
        ...
        elseif not(isequal(size(abmOptions.alpha),[1,1])) || not(isequal(size(abmOptions.betaP),[1,2])) || not(isequal(size(abmOptions.betaC),[1,2]))           % TO DEBUG
...
for i = 2 : (length(t)-1)         % main loop of the method
    ...
    tp = t(i) + h;
    x(:,i+1) = x(:,i) + h/alpha * (betaC(1)*f(xp,tp) + betaC(2)*fk1);   % k+1
    feval = feval + 3*dimSys;
end
