...
%AM3 Summary of this function goes here
...
switch amOptions.method
    ...
    case 'Custom'
        ...
        elseif not(isequal(size(amOptions.alpha),[1,1])) || not(isequal(size(amOptions.beta),[1,3]))            % TO DEBUG
...
for i = 3 : (length(t)-1)         % main loop of the method
    ...
    tp = t(i) + h;
    fp = @(xp) x(:,i) + h/alpha * (beta(1)*f(xp,tp) + beta(2)*fk1 + beta(3)*fk2) - xp;   % k+1
    [x(:,i+1),~,conv,info] = fsolve(fp, x(:,i), amOptions.options);         % initial guess xk
    feval = feval + info.funcCount + 2*dimSys; 
