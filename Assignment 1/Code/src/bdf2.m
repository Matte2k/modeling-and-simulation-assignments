...
%BDF2 Summary of this function goes here
...
switch bdfOptions.method
    ...
    case 'Custom'
        ...
        elseif not(isequal(size(bdfOptions.alpha),[1,1])) || not(isequal(size(bdfOptions.beta),[1,1])) || not(isequal(size(bdfOptions.gamma),[1,2]))            % TO DEBUG
...
for i = 2 : (length(t)-1)         % main loop of the method
    ...
    tp = t(i) + h;
    fp = @(xp) 1/alpha * ( gamma(1)*xk1 + gamma(2)*xk2 + beta*h*f(xp,tp) ) - xp;   % k+1
