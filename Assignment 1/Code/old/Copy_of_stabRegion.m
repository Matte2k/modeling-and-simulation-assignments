...
for i=2:length(alphaVec)     


    
    A0 = Afun(alphaVec(i));                        % Local A matrix value
    A1 = Afun(alphaVec(i-1));                      % Local A matrix value
    %A2 = Afun(alphaVec(i-2));                      % Local A matrix value
    %A3 = Afun(alphaVec(i-3));                      % Local A matrix value
    
    S = @(h) max(abs(eig(F(h,A0,A1)))) - 1;     % Stability problem definition
    ...
    eig_iterVec = eig(A0);   % Eigenvalue of continuos system
